...
if (cluster.isMaster) {
  ...
  (async function crack() {
    ...
    let partnerHash = fs
      ...
      .split("\n");
    let len = partnerHash.length;
    let chunk = Math.floor(len / numCPUs);
    let start = chunk * (process.pid - 1);
    let end = chunk * process.pid;
    partnerHash = partnerHash.slice(start, end);
    let result = [];
